...
class EEGPlotWidget(QWidget):
    def __init__(self, state: AppState):
        ...
        self.state = state
        self.state.label_clicked.connect(self.box_select)
        self.state.spinner_value_changed.connect(self.change_initial_x_lim)
        self.state.goto_input_return_pressed.connect(self.goto_duration)
        self.state.undo_clicked.connect(self.undo_selection)
    ...
    def show_plot(self, raw_eeg, signal_duration, scale):
        """
        Plot the raw eeg in the figure
        """
        self.signal_duration = signal_duration
        self.fig.clear()
        self.axes = self.eep.create_axes(self.fig, scale)
    ...
    def toggle_selector(self):
        if self.rect_selector and self.rect_selector.active:
            ...
            montage_list = self.state.montage_list
            selected_channels = montage_list[
                first_ch : last_ch + 1
            ]
    ...
    def goto_duration(self, t: int):
        ...
        if hasattr(self, 'axes') and self.axes is not None:
            ...
            t = min(self.signal_duration - t_delta, t)
